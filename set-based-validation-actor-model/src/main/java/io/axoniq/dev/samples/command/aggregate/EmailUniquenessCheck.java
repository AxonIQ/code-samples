...
public class EmailUniquenessCheck {
    ...
    public void handle(RegisterUserWithUniqueEmailAddress command) throws Exception {
        // A user should only be registered when the email address is unique. This is the case when the version is zero or the aggregate is deleted
        Assert.isTrue(AggregateLifecycle.getVersion() == null || isDeleted,
    ...
    public void handle(RemoveUserWithUniqueEmailAddress command) {
        Assert.isTrue(!isDeleted, () -> "This email address is already deleted");
