...
public class ProcessOrderSagaTest {
    ...
    void onOrderConfirmedTest() {
        testFixture.givenNoPriorActivity()
                   ...
                   .expectScheduledDeadlineWithName(Duration.of(5, ChronoUnit.DAYS), "OrderCompleteDeadline")
                   .expectActiveSagas(1);
    }
    ...
    void onOrderPaidAndDeliveredTest() {
        testFixture.givenAPublished(new OrderConfirmedEvent(orderId))
                   ...
                   .expectNoScheduledDeadlineWithName(Duration.of(5, ChronoUnit.DAYS), "OrderCompleteDeadline")
                   .expectActiveSagas(0);
    }
    ...
    void onOrderCompleteDeadlineTest() {
        testFixture.givenAPublished(new OrderConfirmedEvent(orderId))
                   .whenTimeElapses(Duration.of(5, ChronoUnit.DAYS).plus(Duration.ofSeconds(60)))
                   .expectNoScheduledDeadlines()
                   .expectTriggeredDeadlinesMatching(orderCompleteDeadline());
    }
    ...
    static class OrderCompleteDeadline extends TypeSafeMatcher<List<DeadlineMessage<?>>> {
        ...
        protected boolean matchesSafely(List<DeadlineMessage<?>> deadlineMessages) {
            return deadlineMessages.stream().allMatch(OrderCompleteDeadline::matches);
        }

        private static boolean matches(DeadlineMessage<?> deadlineMessage) {
            return deadlineMessage.getDeadlineName().equals("OrderCompleteDeadline")
